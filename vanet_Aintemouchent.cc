...
uint32_t RoutingStats::GetRxBytes ()
{
  return m_RxBytes;
}
...
uint32_t RoutingStats::GetTxBytes ()
{
  return m_TxBytes;
}
...
AnimationInterface anime("n.xml");
  Simulator::Run ();
  Simulator::Destroy ();
}
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  if (m_pcap != 0)
    {
      wifiPhy.EnablePcapAll ("vanetF-routing-compare-pcap");
      wavePhy.EnablePcapAll ("vanetV-routing-compare-pcap");
    }
}
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
    ...
    {
      // Realistic vehicular trace in ain_temouchenet 
      // "density= 204 total vehicles"
      m_traceFile = "/MP_RSFE/mobility_mai10.tcl";
      m_logFile = "mob_temouchent.log";
      m_mobility = 1;
      m_nNodes = 204;
      m_TotalSimTime = 60.01;
      m_nodeSpeed = 11;
      m_nodePause = 0;
      m_CSVfileName = "temoouchent.csv";
      m_CSVfileName = "temoouchent2.csv";
    }
